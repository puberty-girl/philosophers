...
long	ft_atol(const char *str)
{
	...
	while (ft_isspace(*str) || *str == '+')
		str++;
	if (*str == '-')
		return (printf("numbers cant be negtive\n"), -1);
	while (*str >= '0' && *str <= '9' && *str)
	{
		num = num * 10 + (*str - '0');
		str++;
		if (num > INT_MAX)
			return (printf("numbers cant be bigger than intmax\n"), -1);
	}
	if (!(*str >= '0' && *str <= '9') && *str != '\0')
		return (printf("incorrect digits\n"), -1);
	return (num);
}
...
int	parse_init(t_table *table, char **argv)
{
	...
	if (table->nbr_of_philos <= 0 || table->time_to_die <= 0
