...
int	isdead(t_philosopher *philosopher)
{
	...
	if (ready_check(&philosopher->philo_mutex, philosopher->isfull))
		return 0;
	elapsed = get_time(MILLISECOND) - get_long(&philosopher->philo_mutex, &philosopher->last_meal_time);
	time_to_die = philosopher->table->time_to_die;//convert back to milliseconds
